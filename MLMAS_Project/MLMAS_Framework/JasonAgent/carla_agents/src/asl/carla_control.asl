...
// debug plan
//+!start(F): traffic_light(F,A,B,C,E,D,K) & info(F,S)
//		<-   .print("===Traffic Light Frame===: ", F);
//			 .print("===Traffic Light Type ===: ", A);
//			 .print("===Traffic Light State===: ", B);
//			 .print("===Traffic Light X===: ", C);
//			 .print("===Traffic Light Y===: ", E);
//			 .print("===Traffic Light Distance===: ", D);
//			 .print("===Traffic Light inbox?===: ", K);
//			 .print("===Traffic Light Speed===: ", S);
//			 .print("======================================: ");
//			 no_action.



// a plan to check if you just left a traffic light, do not slow down
+!start(F): f(F,X,_,_, MinY) & MinY < 2.5 & X < 4.5
			& info(F,Sp) & Sp > 0.5 & block(Nf, M) & Nf < M & ml_control(F,_,St,_,_,_) & traffic_light(F,_,_,_,_,D,_) & D < 8.0
		<-  no_action. // do nothing, let the ML model drive to prevent slowing down and causing the traffic light red infraction.
			//.print("=== NO ACTION for traffic light ===: ", F).


// slow down if an object is detected between 2.5 and 2.0 lidar range
+!start(F): f(F,X,_,_, MinY) & MinY < 2.5 & MinY > 2.0 & X < 4.5
			& info(F,Sp) & Sp > 5.5 & block(Nf, M) & Nf < M & ml_control(F,_,St,_,_,_)
		<-   control(6, 0.0, St, 0.4, false, false, 2). // slow down
			//.print("===Slow Down -- Collision===: ", F);
			//.print("X for collision: ", X);
			//.print("MinY for collision: ", MinY);
			//.print("Block for collision: ", Nf);
			//.print("Block for collision2: ", M);
			//.print("===Slow Down -- Collision===: ", Sp).
...
+!start(F): f(F,X,_,_, MinY) & MinY < 2.0 & X < 4.5
		...
		<-   control(2, 0.0, 0.0, 1.0, false, false, (Sp*3)). // hard break
			//.print("===Avoid Road [Close Crossing Original] Collision===: ", F);
			//.print("X for collision: ", X);
			//.print("MinY for collision: ", MinY);
			//.print("Block for collision: ", Nf);
			//.print("Block for collision2: ", M);
			//.print("===Avoid Road [Close Crossing Original] Collision2===: ", Sp).
...
// 16. A plan to stop the car when the traffic light is yellow.
+!start(F): traffic_light(F,"A","Y",_,_,_,_) & ml_control(F,_,St,_,_,_)
		  & info(F,S)
		<-   control(0, 0.0, St, 1.0, false, false, 5). // Stop
			//.print("===Traffic Light is Yellow: Stop ===: ", F).

// create a plan for yellowlight reverse action and also do the repeat.

// 17. A plan to continue moving if its not red.
+!start(F): traffic_light(F,"A","R",_,_,D,0) & D > 8.0 & D < 15.0 & ml_control(F,_,St,_,_,_)
		  & info(F,S)
		<-   control(4, 0.3, St, 0.0, false, false, (S)); // Move forward
			.print("===Traffic Light is not Red(nb): Move ===: ", F).

+!start(F): traffic_light(F,_,"R",_,_,D,0) & D < 8.0 & ml_control(F,_,St,_,_,_)
		  & info(F,S)
		<-   control(0, 0.0, St, 1.0, false, false, 5). // Stop
			//.print("===Traffic Light is Red(nb): Stop ===: ", F).

+!start(F): traffic_light(F,"A","R",_,_,D,1) & D < 23.5 & ml_control(F,_,St,_,_,_)
		  & info(F,S)
		<-   control(0, 0.0, St, 1.0, false, false, 5). // Stop
			//.print("===Traffic Light is Red(box): Stop ===: ", F).


+!start(F): traffic_light(F,_,"G",_,_,_,1)
		  & info(F,Sp) & ml_control(F,_,St,_,_,_)
		<-   control(5, 0.8, St, 0.0, false, false, 1). // Go forward
			//.print("===Move from Traffic Light Box2: Go===: ", F);
			//.print("==Steer:==", St).
