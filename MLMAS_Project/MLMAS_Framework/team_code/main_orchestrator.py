...
class Orchestrator(AutonomousAgent):
    ...
    def __init__(self, path_to_conf_file):
        ...
        # to record when each takes control
        self.jason_control_game_time = None
        self.ml_control_game_time = None
        self.trigger = 0
        self.driver_dict = {}
    ...
    def run_step(self, input_data, timestamp):
        ...
        if ml_agent == "ML_Models/LAV/team_code/lav_agent.py":
            self.ml_control = self.ml_model.run_step(input_data, timestamp)
            ...
            #                                           throttle=float(0.0),
            #                                            brake= float(1.0)) )
        elif ml_agent == "leaderboard/leaderboard/autoagents/custom_npc_agent.py":
            self.ml_control = self.ml_model.run_step(input_data, timestamp, self._global_plan_world_coord)
            # self.ml_control = (self.ml_model.run_step(input_data, timestamp, self._global_plan_world_coord) if self.should_get_ml_control()
                            # else carla.VehicleControl(steer=float(0.0),
                                                    #   throttle=float(0.0),
                                                    #    brake= float(1.0)) )
        if self.should_send_to_jason()[0]:
                # cnt = self.send_sensor_data()
                cnt = self.send_sensor_data(self.should_send_to_jason()[1])
                # create a file to save who took control
                self.jason_control_game_time = self._vehicle.get_world().get_snapshot().timestamp.elapsed_seconds
                # print("Jason took control at ", self.jason_control_game_time, " seconds with trigger = ", self.trigger)
                self.save_driver_control([self.num_frames, self.jason_control_game_time, "Jason", cnt, self.should_send_to_jason()[1], round(ego_vehicle_speed,3)], driver_file_path)
                if cnt != -1:                             
                    return cnt

        # create a file to save who took control
        self.ml_control_game_time = self._vehicle.get_world().get_snapshot().timestamp.elapsed_seconds
        self.save_driver_control([self.num_frames, self.ml_control_game_time, "ML", self.ml_control, "", round(ego_vehicle_speed,3)], driver_file_path)
    ...
    def save_driver_control(self, driver_data, file_path):
        if not exists(file_path):
            with open(file_path, 'w') as driver_file:
                driver_file.write("frame;time;driver;control;condition;speed\n")

        with open(file_path, 'a') as driver_file:
            driver_file.write("%s;%f;%s;%s;%s;%f\n" % (str(driver_data[0]), driver_data[1], str(driver_data[2]), str(driver_data[3]), str(driver_data[4]), driver_data[5]))
    ...
    def should_send_to_jason(self):
        ...
        cond = cond or (
                        ...
                        self.traffic_light_detection[1] == "Y" or
                        # self.traffic_light_detection[1] == "R" or                        
                        (self.prev_traffic_light_detection == -1
        ...
        # triggering condition code
        if self.front_objects_detection != -1:
            triggering_condition = "front_objects_detection"
        elif self.back_objects_detection != -1:
            triggering_condition = "back_objects_detection"
        elif self.sBack_objects_detection != -1:
            triggering_condition = "sBack_objects_detection"
        elif self.SFront_objects_detection != -1:
            triggering_condition = "SFront_objects_detection"
        elif self.Left_objects_detection != -1:
            triggering_condition = "Left_objects_detection"
        elif self.right_objects_detection != -1:
            triggering_condition = "right_objects_detection"
        # checks if there is a traffic light ahead and its not yet "A" and checks distance to see if its in the traffic light box.
        elif self.traffic_light_detection != -1 and (self.traffic_light_detection[0] == "L" and (self.ahead_traffic_light_distance - self.traffic_light_detection[4]) > 8):
            triggering_condition = "traffic_light_detection in the box"
        elif self.traffic_light_detection != -1 and self.traffic_light_detection[1] == "G":
            triggering_condition = "traffic_light_detection green"
        elif self.traffic_light_detection != -1 and self.traffic_light_detection[1] == "Y":
            triggering_condition = "traffic_light_detection yellow"
        # elif self.traffic_light_detection != -1 and self.traffic_light_detection[1] == "R":
        #     triggering_condition = "traffic_light_detection red"
        # change in traffic light detection
        elif self.traffic_light_detection != -1 and (self.prev_traffic_light_detection == -1 or self.prev_traffic_light_detection != self.traffic_light_detection):
            triggering_condition = "traffic_light_detection change"


        self.prev_traffic_light_detection = self.traffic_light_detection

        return cond, triggering_condition
    ...
    def check_traffic_lights(self):
        ...
        if light_state:
            ...
            # print("traffic distance (A): ", trfc_distance, trafic_light_states(self._last_traffic_light.state))
            traffic_light_detection = ("A", trafic_light_states(self._last_traffic_light.state),
                           ...
                           trfc_distance, self.is_in_traffic_light_box())#trfc_distance, False)
            ...
            if trfc_distance > 6.0:
                ...
                # print("traffic distance (L): ", trfc_distance, trafic_light_states(self._last_traffic_light.state))
            ...
            # else: print("Close to traffic light")
    ...
    def is_in_traffic_light_box(self):
        ...
        # print("x: ", x)
        # print("y: ", y)
        # print("ego_loc(x): ", ego_loc.x)
        # print("ego_loc(y): ", ego_loc.y)
        # print("answer : ", ego_loc.x > min(x) and ego_loc.x < max(x) and ego_loc.y > min(y) and ego_loc.y < max(y))
    ...
    def send_sensor_data(self, tg_cond):

        self.is_control_responce_recieved = False
        self.jason_carla_bridge.send_sensor_data(tg_cond)
        # print("Send msg")
        sleep_time = 0.1
        time_out = 5 ## sec
        i = 0
